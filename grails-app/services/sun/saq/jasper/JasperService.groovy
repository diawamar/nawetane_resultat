...
class JasperService {
    ...
    /**
     * Generate a report based on a single sun.saq.jasper file.
     * @param format , target format
     * @param reportDef , sun.saq.jasper report object
    ...
    /**
     * Generate a single report based on a list of sun.saq.jasper files.
     * @param format , target format
    ...
    /**
     * Forces the Jasper Reports  temp folder to be "~/.grails/.sun.saq.jasper" and ensures that such a folder exists.
    ...
    protected void forceTempFolder() {
        ...
        if (FORCE_TEMP_FOLDER) {
            ...
            File tempFolder = new File(userHomeDir, "/.grails/.sun.saq.jasper")

            // This is the current official means for setting the temp folder for sun.saq.jasper reports to use when compiling
            ...
            System.setProperty("sun.saq.jasper.reports.compile.temp", tempFolder.getAbsolutePath())
    ...
    private JasperPrint generatePrinter(JasperReportDef reportDef) {
        ...
        if (jrDataSource != null) {
            if (resource.getFilename().endsWith('.sun.saq.jasper')) {
