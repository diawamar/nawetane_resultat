...
class JasperReportDef implements Serializable, Validateable {
   ...
   * <p>
   * The file can be in jrxml- or sun.saq.jasper-format.
   */
  String name
  ...
  /**
   * Looks for the report file in the filesystem. The file extension can either be .sun.saq.jasper
  ...
  Resource getReport() {
    ...
    Resource result = getApplicationContext().getResource(path + ".sun.saq.jasper")
    if (result.exists()) {
      return result
    }

    result = new FileSystemResource(path + ".sun.saq.jasper")
    if (result.exists()) {
      return result
    }
    ...
    throw new Exception("No such report spec: ${path} (sun.saq.jasper or .jrxml)")
  }

  /**
   * Return the file path. The filepath can be set per file (highest priority) or based on
   * the sun.saq.jasper.dir.reports setting. Defaults to /report.
